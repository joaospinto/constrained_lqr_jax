...
from constrained_lqr_jax.types import FactorizationInputs, SolveInputs, SolveOutputs
...
def _cost_to_go_sequential(inputs: FactorizationInputs, solve_inputs: SolveInputs):
    ...
    def step(carry, stage):
        ...
        Hws = jnp.block([[Mk + Ak.T @ Pp @ Bk, Ak.T @ Fp.T], [Ek, jnp.zeros((p, p))]])
...
def factor_and_solve(
    ...
    x, u = _forward_primal_sequential(factorization_inputs, solve_inputs, cog)
    y, lam = _recover_duals(factorization_inputs, solve_inputs, x, u)
    return SolveOutputs(X=x, U=u, Y=y, Lam=lam, p=cog[1], k=jnp.zeros_like(u))
...
def factor_and_solve_parallel(
    ...
    Identical solution to :func:`factor_and_solve`, computed with O(log N)-depth
    associative scans for the backward and forward primal sweeps.
    """
...
def solve_general(
    ...
    Set ``parallel=True`` to use the O(log N)-depth parallel algorithm; both
    paths return the same solution.
